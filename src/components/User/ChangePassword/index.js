...
function ChangePassword () {
    ...
    useEffect(() => {
        if (password1.length < 8) setEightCharacters(false); else setEightCharacters(true);
        if (!password1.match(/[a-zA-Z]/)) setOneLetter(false); else setOneLetter(true);
        if (!password1.match(/\d/)) setOneNumber(false); else setOneNumber(true);
    }, [password1])
    useEffect(() => {
        if(password1 === password2) setPasswordsMatch(true); else setPasswordsMatch(false)
    }, [password1, password2])
                    ...
                    <div className='col-5 mt-3 mb-5 color_third'>
                        ...
                        <div className='px-5 mt-5'>
                            ...
                            <p>
                                ...
                                [{eightCharacters ? '✔️': '❌'}]  contain at least <strong>8 characters</strong>.<br/>
                                [{oneLetter ? '✔️': '❌'}] contain at least <strong>one letter</strong>.<br/>
                                [{oneNumber ? '✔️': '❌'}] contain at least <strong>one number</strong>.<br/>
                                [{passwordsMatch ? '✔️': '❌'}] <strong>Be the same in the two fields</strong>.<br/>
                            </p>
                            <input
                            type='text'
